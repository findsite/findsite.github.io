	...
	function start() {
		...
		coefs[0] = window.localStorage.formulaA;
		coefs[1] = window.localStorage.formulaB;
		coefs[2] = window.localStorage.formulaC;
		...
		text += (signOf(coefs[0]) == '-' ? '-' : '') + (Math.pow(coefs[0],2) == 1 ? '' : toPos(coefs[0])) + 'x²' + signOf(coefs[1]) + (Math.pow(coefs[1],2) == 1 ? '' : toPos(coefs[1])) + 'x' + signOf(coefs[2]) + toPos(coefs[2]) + ' = 0<br/>';
